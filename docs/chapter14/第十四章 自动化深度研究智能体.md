...
此次系统仍然采用经典的**前后端分离架构**，如图 14.1 所示。
  ...
  ![图 14.1 深度研究助手技术架构](/images/14-figures/14-1.png)
...
让我们看看一个完整的研究请求是如何在系统中流转的，如图 14.2 所示：
  ...
  ![图 14.2 深度研究助手数据流转过程](/images/14-figures/14-2.png)
...
打开浏览器访问 `http://localhost:5174`，你会看到一个居中的输入卡片，如图 14.3 所示。输入研究主题，例如`Datawhale是一个什么样的组织？`，选择搜索引擎（如果配置了多个），点击"开始研究"按钮。
  ...
  ![图 14.3 深度研究助手搜索页面](/images/14-figures/14-3.png)
...
如图 14.4 所示，系统会自动展开为全屏，左侧显示研究信息，右侧实时显示研究进度和结果。整个研究过程大约需要 1-3 分钟，取决于主题的复杂度和搜索引擎的响应速度。
  ...
  ![图 14.4 深度研究助手展开研究](/images/14-figures/14-4.png)
...
在我们的案例里，TODO 驱动的研究流程如图 14.5 所示：
  ...
  ![图 14.5 TODO 驱动的研究流程](/images/14-figures/14-5.png)
...
三个 Agent 之间是**顺序协作**的关系，如图 14.6 所示。
  ...
  ![图 14.6 Agent 协作流程](/images/14-figures/14-6.png)
...
当 Agent 需要调用工具时，它会生成工具调用指令，如图 14.7 所示。
  ...
  ![图 14.7 工具调用流程](/images/14-figures/14-7.png)
...
根据配置选择搜索引擎，如图 14.8 所示：
  ...
  ![图 14.8 搜索引擎调度流程](/images/14-figures/14-8.png)
...
如图 14.9 所示，全屏模态对话框包含以下部分：
  ...
  ![图 14.9 全屏模态对话框 UI](/images/14-figures/14-9.png)
...
如图 14.10 所示，SSE 流程包括以下步骤：
  ...
  ![图 14.10 SSE 流程](/images/14-figures/14-10.png)
